...
##
## [b]The mode filter is the one thing here that is this game's.[/b] `hungry_gauntlet` and
## `hungry_classic` are different games in the way that matters to somebody choosing a
## server, and dot-server reports the running game id in a query — so a person who only
## wants the corridor can have only the corridor, without a server having to be asked.

const CHANNEL := "hungry.browser"

const DEFAULT_PORT := 27081
...
func _build() -> void:
	...
	_mode.add_item("Any mode", 0)
	var index := 1

	for descriptor in HungryModule.game_descriptors():
		_mode.add_item(descriptor.display_name, index)
		_mode.set_item_metadata(index, descriptor.game_id)
		index += 1
...
func _start() -> void:
	...
	browser.register_as = &""
	browser.favourites_path = "user://hungry_servers.json"
	add_child(browser)
...
func _apply_filter() -> void:
	...
	browser.filter.game_id = str(wanted) if wanted != null else ""
	_redraw()
...
func _redraw() -> void:
	...
	for entry in _rows:
		rows.append({
			...
			&"mode": entry.game_id if entry.game_id != "" else entry.map,
...
func _show_selection() -> void:
	...
	if not entry.is_online():
		...
		_status.text = "%s — %s, %d of %d, %d ms." % [
			entry.name, entry.game_id, entry.players, entry.max_players, entry.ping_ms
		]
