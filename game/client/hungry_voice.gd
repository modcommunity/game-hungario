...
## key-up delivered into a chat box is a gate that never closes, and the resulting bug is
## reported as "everyone could hear me" rather than as a bug.
##
...
## `AudioServer` reports a 44100 Hz mix rate and a "Default" input device in a headless
## run and only `get_driver_name()` says `"Dummy"`. A capability check built on any of the
## others passes on a machine with no audio at all, and the symptom is a capture that
## returns silence for ever with nothing reporting a problem.

const CHANNEL := "hungry.voice"

## Held to talk. `V`, because it is not a movement key and this game's are the mouse.
...
func setup(enable_capture: bool = true) -> DotResult:
	...
	manager.config = HungryServices.voice_config()
	manager.config_file = ""
