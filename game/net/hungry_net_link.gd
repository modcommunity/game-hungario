...
## Counters, for `hungry_net` and for the self-test.
...
static func attached_to(parent: Node, p_bridge: HungryNetBridge, server: bool) -> HungryNetLink:
	var link := HungryNetLink.new()
	link.name = NODE_NAME
...
##
## [b]One call for both transports, because there is only one question and the answer
## differs by transport rather than by code.[/b] On ENet `unreliable` is a UDP datagram
## that is never retransmitted, which is what voice wants — a frame that arrives late is
## one the jitter buffer has already concealed. On a WebSocket every transfer mode is TCP
## underneath and it is delivered reliably whether or not that was asked for. That is a
## property of the transport, not a gap here.
##
...
## [method MultiplayerAPI.get_remote_sender_id] is the fact, and
